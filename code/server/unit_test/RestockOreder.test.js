...
describe('get SKU by ID',()=>{
    ...
    test('get SKU by ID',async()=>{
        await expect(RO.insertProductInOrder(1,1,1,10)).resolves.toBe(null);
        await expect(RO.getSKUByIdFromRestockOrder(1,1)).resolves.toEqual({"description": null,"itemID": 1,"quantity": 10,"restockOrderID": 1,"skuID": 1});
    });
...
describe('Add and get Transport Note',()=>{
    ...
    test('Add transport note',async()=>{
        ...
        await expect(RO.getRestockOrderTransportNote(1)).resolves.toEqual({"TransportNote":{'DeliveryDate':'19/11/1997'}});
    });
