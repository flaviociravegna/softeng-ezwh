...
describe('ReturnOrder Products', ()=>{
    ...
    test('get return order products',async()=>{
        await expect(RO.getReturnOrderProducts(1)).resolves.toEqual([{'SKUId':1,'description':"a description.",'price':19.99, 'RFID':'12341234'}]);
    });
    ...
    test('fail to insert a repeated product',async()=>{
        await expect(RO.insertProductInRO({'SKUId':1,'description':"a description.",'price':19.99, 'RFID':12341234},1)).rejects.toThrow();    
    });
    test('insert a product',async()=>{
        await expect(RO.insertProductInRO({'SKUId':2,'description':"another description.",'price':29.99, 'RFID':222222},1)).resolves.toBe('inserted products');    
        
        await expect(RO.getReturnOrderProducts(1)).resolves.toEqual([{'SKUId':1,'description':"a description.",'price':19.99, 'RFID':'12341234'},
                                        {'SKUId':2,'description':"another description.",'price':29.99, 'RFID':'222222'}]);
    });
...
describe('delete a return order',()=>{
    ...
    test.only('delete a return Order that doesnt exist',async()=>{
